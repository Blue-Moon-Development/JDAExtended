...
public class Config {
    ...
    private static final Log LOG = Log.get("JDAExtended", Config.class);
    ...
    public Config(String fileName) {
        ...
        if (f.isNewlyCreated()) {
            f.close();
            LOG.info("Configuration file created. Please fill out the details before running again");
            System.exit(0);
        }
        lines = f.readLines();
        f.close();
    ...
    private String[] setupDefaultOptions() {
        ...
        contents[5] = "# If using mysql, fill out the following details, if using "
                + "sqlite, comment out the lines with \'#\'";
        ...
        contents[12] = "database.timezone = <the timezone of the server the database"
                + " is hosted on. For example, if EST, then put EST5EDT>";
        contents[13] = "";
        ...
        contents[21] = "";
        contents[22] = "# Developer ID - Used for developer commands, such as <shutdown> which "
                + "lets you close the bot from a discord server";
    ...
    public int getInt(String key) {
        ...
        LOG.warn(new JDAConfigurationException(), "Option [" + key + "] was expected to be an integer, but it was not");
        return -1;
    }

    public long getLong(String key) {
        if (Checks.isLongNumber(options.get(key))) return Long.parseLong(options.get(key));
        LOG.warn(new JDAConfigurationException(), "Option [" + key + "] was expected to be an long, but it was not");
        return -1L;
    }
